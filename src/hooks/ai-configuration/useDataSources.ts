...
export function useDataSources() {
  ...
  const fetchDataSources = useCallback(async () => {
    ...
    try {
      ...
      setError(
        err instanceof Error ? err : new Error("Failed to fetch data sources"),
      );
  ...
  const handleSaveNewDataSource = async (
    newDataSource: Omit<DataSource, "id">,
  ) => {
    setIsSaving(true);
    try {
  ...
  const handleSaveEditedDataSource = async (editedDataSource: DataSource) => {
    ...
    try {
      ...
      setDataSources(
        dataSources.map((ds) => (ds.id === updated.id ? updated : ds)),
      );
